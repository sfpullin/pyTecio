...
std::cerr << "Warning: Uncompleted MPINonBlockingSendScalarCopy going out of scope." << std::endl;
 #endif
} } template <typename T> bool tecplot::teciompi::MPINonBlockingCommunicationCollection::MPINonBlockingSendScalarCopy<T>::isComplete() { return m_sendScalar.isComplete(); } template <typename T> void tecplot::teciompi::MPINonBlockingCommunicationCollection::MPINonBlockingSendScalarCopy<T>::___4444() { m_sendScalar.___4444(); } template <typename T> tecplot::teciompi::MPINonBlockingCommunicationCollection::MPINonBlockingReceiveScalar<T>:: MPINonBlockingReceiveScalar(T& ___4296, int ___3654, int tag, MPI_Comm comm)
 #if defined _DEBUG
...
std::cerr << "Warning: Uncompleted MPINonBlockingSendVectorCopy going out of scope." << std::endl;
 #endif
} } template <typename T> bool MPINonBlockingCommunicationCollection::MPINonBlockingSendVectorCopy<T>::isComplete() { return m_sendVector.isComplete(); } template <typename T> void MPINonBlockingCommunicationCollection::MPINonBlockingSendVectorCopy<T>::___4444() { m_sendVector.___4444(); } template <typename T> MPINonBlockingCommunicationCollection::MPINonBlockingReceiveVector<T>::MPINonBlockingReceiveVector( SimpleVector<T>& vec, int ___3654, int sizeTag, int vecTag, MPI_Comm comm)
 #if defined _DEBUG
...
for (RequestVector_t::const_iterator request = m_requests.begin(); request != m_requests.end(); ++request) if (!(*request)->isComplete()) return false; return true;
 #endif
} void MPINonBlockingCommunicationCollection::___4444() { size_t numRequests = m_requests.size(); for (size_t i = 0; i < numRequests; ++i) { bool complete = true; for (size_t ___2103 = 0; ___2103 < numRequests; ++___2103) { complete = m_requests[___2103]->isComplete() && complete; } if (complete) break; } for (RequestVector_t::const_iterator request = m_requests.begin(); request != m_requests.end(); ++request) (*request)->___4444(); m_requests.clear(); }
